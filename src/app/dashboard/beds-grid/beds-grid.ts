...
export class BedsGrid {
  ...
  statusLabel(status: IcuBedStatus): string {
    return status === 'AVAILABLE' ? 'Disponible' : 'Ocupada';
  }

  /* WCAG SC 1.4.1: shape glyph so status is never conveyed by color alone.
   * ● available (circle) · ■ occupied (square) */
  ...
  cardClasses(status: IcuBedStatus): string {
    return status === 'AVAILABLE'
      ? 'bg-green-800 text-green-50 ring-green-500/40 hover:bg-green-700'
      : 'bg-red-800 text-red-50 ring-red-500/40';
  }

  badgeClasses(status: IcuBedStatus): string {
    return status === 'AVAILABLE'
      ? 'bg-green-950/60 text-green-100 border-green-300/30'
      : 'bg-red-950/60 text-red-100 border-red-300/30';
  }
}
